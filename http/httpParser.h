#ifndef __BLUE_HTTP_PARSER_H__
#define __BLUE_HTTP_PARSER_H__
#include <memory>
#include <map>
#include <string>
#include <iostream>
...
namespace blue
{
    namespace http
    {
        ...
        public:
             ...
             * @param cb 回调函数
             */
            void on_Body(std::function<int()> cb);

            /**
             * @brief 整条 HTTP 消息解析完成后触发,意味着解析完成
             * @param cb 回调函数
             * @note 如果http请求支持长连接那么就会在调用完用户设定的回调函数后，把所有数据清空
             */
