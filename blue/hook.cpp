...
namespace blue
{
	...
	blue::Config::Lookup<int>("tcp.connect.timeout",5,"tcp connect timeout");
	...
	*/
		...
		while (true)
		{
			...
			if (n == -1 && errno == EAGAIN)
			{
				...
				*/
				...
				{
					timer = iom->addConditionTimer(timeout * 1000ul,[weakPtr = weakTimerInfo,
						im = iom,fd_ = fd,event_ = event](){
							...
							im->cancelEvent(fd_,(blue::IOManager::Event)event_);
					},weakTimerInfo);
				}
	...
	int connect_with_timeout(int sockfd,const struct sockaddr* addr,socklen_t len,uint64_t timeout)
	{
		...
		if (timeout != UINT64_MAX)
		{
			timer = iom->addConditionTimer(timeout * 1000ul,
				[im = iom,weak = weak_timerinfo,fd = sockfd](){
					blue::Logger::LoggerPtr g_logger = BLUE_LOG_NAME("system");
					...
					sharPtr->cancelled.store(ETIMEDOUT,std::memory_order_release);
					im->cancelEvent(fd,blue::IOManager::Event::WRITE);
				},weak_timerinfo);
		}
		// 提交任务给epoll
		...
		// 到这里调用getsockopt返回并清楚被挂起的套接字错误,如果设置error为0,表示tcp三次握手真正成功
